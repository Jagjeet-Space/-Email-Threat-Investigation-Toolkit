...
# Force dnspython to use reliable public DNS servers (Google + Cloudflare)
dns.resolver.default_resolver = dns.resolver.Resolver(configure=True)
dns.resolver.default_resolver.nameservers = ['8.8.8.8', '1.1.1.1']
dns.resolver.default_resolver.lifetime = 10   # Total query lifetime
dns.resolver.default_resolver.timeout = 5     # Per retry timeout
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python analyze_headers.py <path_to_eml_file>", file=sys.stderr)
        sys.exit(1)
